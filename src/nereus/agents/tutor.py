...
class TutorAgent(BaseAgent):
    ...
    an LLM provider is available the material is generated by the model with
    strict JSON schema validation and bounded retries; otherwise deterministic
    stubs are used. On a ``RETRY`` verdict it produces a revision block focused
    on the reported weak areas.
    """
